                ...
                if (i <= channelManager.trackScheduler.queue.size()) {
                    if (p >= 1) {
                        if (p <= channelManager.trackScheduler.queue.size() + 1) {
                            AudioTrack[] array = new AudioTrack[channelManager.trackScheduler.queue.size()];
                            channelManager.trackScheduler.queue.toArray(array);
                            List<AudioTrack> list = new ArrayList<>(Arrays.asList(array));
                            AudioTrack track = list.get(i - 1);

                            if (i >= p) {
                                list.add(p - 1, track.makeClone());
                                list.remove(i);
                            } else {
                                list.add(p - 1, track.makeClone());
                                list.remove(i - 1);
                            ...
                            channelManager.trackScheduler.queue = new ConcurrentLinkedQueue<>(list);
